...
export function callYelpApi(query) {
  ...
  const promise = fetch(`${API_URL}/businesses/search?sort_by=${query.sorting}&limit=20&location=${query.location.length > 0 ? query.location : 'NYC'}&term=${query.search}`, options)
